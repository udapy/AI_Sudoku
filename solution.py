...
def naked_twins(values):
    ...
    # Finding all naked twins and eliminate the naked twins
    twins_seen = set()
    for box, val in values.items():
        ...
        if len(val) == 2 and box not in twins_seen:
            # finding similar values of peers
            twin = next((peer for peer in peers[box] if values[peer] == val), None)
            if twin:
                twins_seen |= {twin} # adding twins tupple for future refrence
                #Checking two values from adjacent peers 
...
diagonal_units = [[rows[i] + cols[i] for i in range(len(rows))],
                  [rows[-(i+1)] + cols[i] for i in range(len(rows))]] # add diagonal units
